...
namespace kms.Controllers
{
    ...
    public class ProjectsController : Controller
    {
        ...
        public ProjectsController(KMSDBContext context, IKMSDBConnection connection, ISearchRepository search)
        {
        ...
        public async Task<IActionResult> GetList([FromQuery] int? user, [FromQuery] bool? my, [FromQuery] int? offset, [FromQuery] int? limit, [FromQuery] string query)
        {
            var sqlquery = @"select p.*,
                (select count(distinct user_id) from project_team as pt where pt.project_id = p.project_id limit 1) as members_count
                from projects as p
                left join project_team as pt on pt.project_id = p.project_id";

            if (query.IsValidQuery()) {
                sqlquery += @" where p.name % @query " + (user.HasValue || my.HasValue && my.Value ? "and pt.user_id = @user" : "") + @" group by p.project_id order by p.name <-> @query";
            } else {
                sqlquery += (user.HasValue || my.HasValue && my.Value ? @" where pt.user_id = @user" : "") + @" group by p.project_id order by p.project_id desc";
            }

            var projects = await _dbconnection.QueryAsync<ProjectShortDto>(sqlquery, new {
                user = user.HasValue ? user.Value : (my.HasValue && my.Value ? UserId : 0),
                query
            });

            return Ok(new {
                count = projects.Count(),
                results = projects.Skip(offset.HasValue ? offset.Value : 0).Take(limit.HasValue ? limit.Value : 50)
            });
        }
